...
const MobileGrid = ({ onSelect }) => (
  <div style={{
    ...
    {/* Header */}
    <div style={{ textAlign: 'center', padding: '4px 20px 10px', flexShrink: 0 }}>
      <h1 style={{
        fontFamily: 'Inter, sans-serif', fontSize: '26px', fontWeight: 300,
        color: C.primary, margin: 0, lineHeight: 1.1, letterSpacing: '-0.5px',
      }}>
        <span style={{ fontWeight: 700 }}>BRIDGE</span> <span style={{ fontWeight: 700, color: C.accent }}>Apps</span>
      </h1>
      <p style={{ fontSize: '11px', color: C.body, margin: '4px 0 0', lineHeight: 1.4 }}>
        Real tools. Real impact. Built for Ghana.
      </p>
    </div>

    {/* Grid — scrollable 3 col */}
    <div style={{
      ...
      {apps.map((app) => {
        ...
        return (
          <div
            ...
            style={{
              backgroundColor: C.white, border: `1px solid ${C.line}`, borderRadius: 10,
              padding: 10, cursor: app.route ? 'pointer' : 'default',
    ...
    <div style={{
      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      padding: '8px 20px', borderTop: `1px solid ${C.line}`, flexShrink: 0,
    }}>
      <span style={{ fontSize: '8px', color: C.label }}>{'\u00A9'} 2026 BRIDGE PBC</span>
      <span style={{ fontSize: '8px', color: C.label }}>BRIDGE Apps</span>
    </div>
  </div>
);
