    ...
    <footer style={{ background: C.card, borderTop: `1px solid ${C.border}`, padding: '40px 16px 28px', width: '100%' }}>
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
          <a href="https://bridgepbc.com" target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: C.t4, textDecoration: 'none' }}>Privacy</a>
          <a href="https://bridgepbc.com" target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: C.t4, textDecoration: 'none' }}>Terms</a>
      ...
      <div>
        ...
        <div className="rg-footer-bottom" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 24 }}>
            <FooterLink href="https://bridgepbc.com" mono external color={C.t4}>Privacy Policy</FooterLink>
            <FooterLink href="https://bridgepbc.com" mono external color={C.t4}>Terms of Service</FooterLink>
...
// ── FIX 11: LOGIN MODAL — fixed className conflict, ensured fade-up ────────
      ...
      <div onClick={e => e.stopPropagation()} className="rg-modal"
        ...
        <div style={{ padding: '0 28px 28px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ textAlign: 'center', fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: '#2A4030', lineHeight: 1.6 }}>
            ...
            <span style={{ color: '#0FA86A', cursor: 'pointer', textDecoration: 'underline', textDecorationStyle: 'dotted' }} onClick={onClose}>Request access →</span>
          </div>
        </div>
      </div>
    </div>
...
// ── FIX 12: DEMO MODAL — fixed className conflict, ensured fade-up ─────────
const DemoModal = ({ onClose }) => {
  ...
  const roles = ['Ministry / Government Official','Rent Control Department','Municipal/District Authority','Real Estate / Property Sector','Financial Institution','NGO / Civil Society','Research / Academia','Press / Media','Other'];
  ...
  return (
    <div onClick={onClose} role="dialog" aria-label="Request a demo"
      ...
      {/* FIX 12: Combined className into style animation to avoid overwrite */}
      <div onClick={e => e.stopPropagation()} className="rg-modal"
        style={{ width: '100%', maxWidth: 540, background: '#111A14', border: '1px solid #1E3023', borderRadius: 20, overflow: 'hidden', boxShadow: '0 40px 120px rgba(0,0,0,0.8), 0 0 0 1px rgba(184,247,60,0.06)', maxHeight: '90vh', overflowY: 'auto', animation: 'fadeUp 0.4s cubic-bezier(0.22,1,0.36,1) both' }}>
        {step === 1 && (
          <>
            <div style={{ padding: '28px 32px 0', position: 'relative' }}>
              <button onClick={onClose} aria-label="Close dialog"
                style={{ position: 'absolute', top: 20, right: 20, width: 32, height: 32, borderRadius: '50%', background: '#1E3023', border: 'none', color: '#4E7055', fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', minHeight: 32 }}
                onMouseEnter={e => { e.currentTarget.style.background = '#2A4030'; e.currentTarget.style.color = '#E8F2EB'; }}
                onMouseLeave={e => { e.currentTarget.style.background = '#1E3023'; e.currentTarget.style.color = '#4E7055'; }}>✕</button>
              ...
              <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 34, color: '#E8F2EB', lineHeight: 1, marginBottom: 8, letterSpacing: '-0.5px' }}>Request a Demo</div>
              <p style={{ fontSize: 14, color: '#9BB8A3', lineHeight: 1.65, marginBottom: 24, paddingRight: 40 }}>We'll set up a live walkthrough tailored to your role — ministry, enforcement, landlord, or tenant services.</p>
            ...
            <div style={{ padding: '0 32px 28px' }}>
              <div className="rg-modal-2col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div style={F.field}>
                  ...
                  <input value={form.name} onChange={e => upd('name', e.target.value)} placeholder="Ama Antwi"
                    style={{ ...F.input, ...(errors.name ? F.inputErr : {}) }}
                    onFocus={e => e.target.style.borderColor = '#0FA86A'} onBlur={e => e.target.style.borderColor = errors.name ? '#E5483A' : '#1E3023'} />
                  {errors.name && <div style={F.err}>{errors.name}</div>}
                </div>
                <div style={F.field}>
                  <label style={F.label}>Organisation *</label>
                  <input value={form.org} onChange={e => upd('org', e.target.value)} placeholder="Ministry of Works & Housing"
                    style={{ ...F.input, ...(errors.org ? F.inputErr : {}) }}
                    onFocus={e => e.target.style.borderColor = '#0FA86A'} onBlur={e => e.target.style.borderColor = errors.org ? '#E5483A' : '#1E3023'} />
              ...
              <div style={F.field}>
                ...
                <select value={form.role} onChange={e => upd('role', e.target.value)}
                  ...
                  onFocus={e => e.target.style.borderColor = '#0FA86A'} onBlur={e => e.target.style.borderColor = errors.role ? '#E5483A' : '#1E3023'}>
              ...
              <div className="rg-modal-2col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div style={F.field}>
                  ...
                  <input type="email" value={form.email} onChange={e => upd('email', e.target.value)} placeholder="ama@rentcontrol.gov.gh"
                    style={{ ...F.input, ...(errors.email ? F.inputErr : {}) }}
                    onFocus={e => e.target.style.borderColor = '#0FA86A'} onBlur={e => e.target.style.borderColor = errors.email ? '#E5483A' : '#1E3023'} />
                ...
                <div style={F.field}>
                  ...
                  <input type="tel" value={form.phone} onChange={e => upd('phone', e.target.value)} placeholder="024XXXXXXX" style={F.input}
                    onFocus={e => e.target.style.borderColor = '#0FA86A'} onBlur={e => e.target.style.borderColor = '#1E3023'} />
                </div>
              </div>
              <div style={F.field}>
                ...
                <textarea value={form.message} onChange={e => upd('message', e.target.value)} rows={3}
                  placeholder="e.g. We need to track advance violations across Greater Accra…"
                  style={{ ...F.input, resize: 'vertical', lineHeight: 1.6 }}
                  onFocus={e => e.target.style.borderColor = '#0FA86A'} onBlur={e => e.target.style.borderColor = '#1E3023'} />
              </div>
              <button onClick={submit} disabled={submitting} aria-label={submitting ? 'Submitting request' : 'Submit demo request'}
                style={{ width: '100%', padding: '14px 24px', background: submitting ? '#2A4030' : '#B8F73C', color: submitting ? '#4E7055' : '#060D08', border: 'none', borderRadius: 10, fontFamily: "'Space Grotesk', sans-serif", fontSize: 15, fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, minHeight: 48 }}>
                {submitting ? (<><span style={{ width: 16, height: 16, border: '2px solid #4E7055', borderTopColor: '#9BB8A3', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.8s linear infinite' }} />Submitting…</>) : 'Request Demo →'}
              </button>
              <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: '#2A4030', textAlign: 'center', marginTop: 14, lineHeight: 1.6 }}>We respond within 1 business day · No spam · Unsubscribe anytime</p>
            </div>
          </>
        )}
        {step === 2 && (
          <div style={{ padding: '52px 40px', textAlign: 'center' }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(184,247,60,0.1)', border: '2px solid rgba(184,247,60,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', fontSize: 28 }}>✓</div>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 40, color: '#E8F2EB', letterSpacing: '-0.5px', lineHeight: 1, marginBottom: 14 }}>Request Received</div>
            ...
            <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: '#4E7055', marginBottom: 36 }}>RentGuard · BRIDGE PBC · Ghana-First</p>
            <button onClick={onClose} aria-label="Return to landing page"
              style={{ padding: '12px 32px', background: 'transparent', border: '1px solid #1E3023', borderRadius: 8, color: '#9BB8A3', fontFamily: "'Space Grotesk', sans-serif", fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', minHeight: 44 }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#B8F73C'; e.currentTarget.style.color = '#B8F73C'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = '#1E3023'; e.currentTarget.style.color = '#9BB8A3'; }}>Back to RentGuard</button>
          </div>
        )}
      </div>
    </div>
...
export default function RentGuardLanding() {
  ...
  const [showPitchToast, setShowPitchToast] = useState(false);
  ...
  const handlePitchDeck = () => {
    setShowPitchToast(true);
    setTimeout(() => setShowPitchToast(false), 3000);
  };
  ...
  return (
    <div id="top" style={{ width: '100%', minWidth: 0, overflowX: 'hidden' }}>
      ...
      {showPitchToast && (
        <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 6000, background: C.card, border: `1px solid ${C.lime}40`, borderRadius: 12, padding: '14px 24px', boxShadow: '0 16px 48px rgba(0,0,0,0.6)', animation: 'fadeUp 0.3s ease both', display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: C.lime, animation: 'pulse 2s infinite' }} />
          <div>
            <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, fontWeight: 600, color: C.t1 }}>Pitch deck download coming soon</div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: C.t3, marginTop: 2 }}>Request a demo for early access to the full deck</div>
          </div>
        </div>
      )}
      ...
      <EndorsementQuote />
      <IntegrationStrip />
      <CTASection onDemo={openModal} onPitchDeck={handlePitchDeck} />
      <Footer />
    </div>
  );
}
