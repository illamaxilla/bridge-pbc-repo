...
// ── Site Navigation Items (matches SiteHeader) ───────────────
const NAV_ITEMS = [
  { label: "Home",               to: "/" },
...
// ═══════════════════════════════════════════════════════════════
function MobileGrid({ onSelect, menuOpen, onMenuToggle, navigate }) {
  ...
  return (
    <div style={{
      ...
      <MenuOverlay open={menuOpen} onClose={onMenuToggle} navigate={navigate} />
...
// DESKTOP — Grid
// ═══════════════════════════════════════════════════════════════
const DesktopGrid = ({ hovered, setHovered, onSelect, menuOpen, onMenuToggle, navigate }) => (
  <div style={{
    width: '100vw', height: '100vh', overflow: 'hidden',
    backgroundColor: C.bg, fontFamily: 'Inter, sans-serif',
    display: 'flex', flexDirection: 'column',
...
export default function BRIDGEAppsLanding() {
  ...
  if (isMobile) {
    return selectedApp
      ...
      : <MobileGrid onSelect={onSelect} menuOpen={menuOpen} onMenuToggle={onMenuToggle} navigate={navigate} />;
  }

  return selectedApp
    ? <DesktopCountdown app={selectedApp} onBack={onBack} timeLeft={timeLeft} />
    : <DesktopGrid hovered={hovered} setHovered={setHovered} onSelect={onSelect} menuOpen={menuOpen} onMenuToggle={onMenuToggle} navigate={navigate} />;
}
