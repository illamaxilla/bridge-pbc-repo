...
const Gf=()=>(<style>{`
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  @media(max-width:600px){
    ...
    .cal-row{grid-template-columns:70px 1fr!important;}
    ...
    .nav-clearance{padding-bottom:68px!important;}
    /* Upsell mobile */
    .upsell-tc{grid-template-columns:1fr!important;}
    .upsell-col-r{padding-left:0!important;border-left:none!important;padding-top:20px!important;}
  }
`}</style>);
...
const SECS=[
  ...
  {id:'sec-tracker', label:'Policy Tracker'},
  {id:'sec-calendar',label:'Regulatory Calendar'},
  {id:'sec-matrix',  label:'Sector Matrix'},
...
const TopBar=({coverLogoRef})=>{
  ...
  return(
    <div className="no-print pad-topbar" style={{position:'sticky',top:0,zIndex:100,background:C.paper,borderBottom:`1px solid ${C.border}`,padding:'10px 48px',display:'flex',justifyContent:'space-between',alignItems:'center',boxShadow:'0 1px 6px rgba(0,0,0,0.05)',overflow:'hidden'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:'10px',minWidth:0,overflow:'hidden'}}>
        <div style={{overflow:'hidden',maxWidth:past?'160px':'0px',opacity:past?1:0,transition:'max-width 0.35s ease,opacity 0.3s ease',display:'flex',alignItems:'center',flexShrink:0}}>
        ...
        <span className="mob-hide" style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:C.muted,background:C.paperDark,padding:'3px 8px'}}>Public Edition</span>
      </div>
...
const SectionFooterNav=()=>{
  ...
  const goTo=(idx)=>{
    const c=Math.max(0,Math.min(SECS.length-1,idx));
    const el=document.getElementById(SECS[c].id);
    if(el)el.scrollIntoView({behavior:'smooth',block:'start'});
    setActive(c);
  };
  return(
    <div className="no-print" style={{position:'fixed',bottom:0,left:0,right:0,zIndex:200,background:'rgba(13,26,16,0.97)',borderTop:`1px solid rgba(184,217,53,0.12)`,backdropFilter:'blur(10px)',WebkitBackdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 20px',gap:'12px'}}>
      <button onClick={()=>goTo(active-1)} disabled={active===0} style={{width:'34px',height:'34px',flexShrink:0,background:active===0?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.1)',cursor:active===0?'default':'pointer',display:'flex',alignItems:'center',justifyContent:'center',opacity:active===0?0.3:1,transition:'all 0.2s'}}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={C.lime} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
      </button>
      <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:'7px',minWidth:0,overflow:'hidden'}}>
        <div style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:'rgba(255,255,255,0.45)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:'100%'}}>
          <span style={{color:C.lime,marginRight:'6px'}}>§ {String(active+1).padStart(2,'0')}</span>{SECS[active]?.label}
        </div>
        <div style={{display:'flex',gap:'5px',alignItems:'center'}}>
          {SECS.map((_,i)=>(<div key={i} onClick={()=>goTo(i)} style={{width:i===active?'20px':'6px',height:'6px',borderRadius:'3px',background:i===active?C.lime:'rgba(255,255,255,0.18)',cursor:'pointer',transition:'all 0.3s ease',flexShrink:0}}/>))}
        </div>
      </div>
      <button onClick={()=>goTo(active+1)} disabled={active===SECS.length-1} style={{width:'34px',height:'34px',flexShrink:0,background:active===SECS.length-1?'rgba(255,255,255,0.03)':C.forest,border:`1px solid ${active===SECS.length-1?'rgba(255,255,255,0.1)':'rgba(184,217,53,0.2)'}`,cursor:active===SECS.length-1?'default':'pointer',display:'flex',alignItems:'center',justifyContent:'center',opacity:active===SECS.length-1?0.3:1,transition:'all 0.2s'}}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={C.lime} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
      </button>
    </div>
  );
};
...
const Cover=({logoRef})=>(
  <div id="sec-cover" className="pad-cover" style={{background:C.ink,padding:'32px 64px 0',position:'relative',overflow:'hidden'}}>
    ...
    <div style={{height:'3px',background:C.lime,marginTop:'0'}}/>
  </div>
);
...
const PolicyIntelligence=({filtered,open,setOpen})=>{
  ...
  return(
  <div id="sec-tracker" className="pad-section" style={{background:C.paper,padding:'28px 64px',borderBottom:`1px solid ${C.border}`}}>
...
const RegulatoryCalendar=()=>{
  ...
  return(
    <div id="sec-calendar" className="pad-section" style={{background:C.paperDark,padding:'48px 64px',borderBottom:`1px solid ${C.border}`}}>
      <div style={{maxWidth:'900px',margin:'0 auto'}}>
        ...
        <div>
          {events.map((e,i)=>{
            ...
            return(
              <div key={i} className="cal-row" style={{display:'grid',gridTemplateColumns:'80px 1fr auto',gap:'16px',alignItems:'start',padding:'11px 0',borderBottom:i<events.length-1?`1px solid ${C.border}`:'none'}}>
...
const SectorMatrix=()=>(
  <div id="sec-matrix" className="pad-section" style={{background:C.paper,padding:'48px 64px',borderBottom:`1px solid ${C.border}`}}>
    <div style={{maxWidth:'900px',margin:'0 auto'}}>
      <div style={{borderTop:`6px solid ${C.ink}`,borderBottom:`2px solid ${C.lime}`,paddingBottom:'3px',marginBottom:'18px'}}/>
...
const Gate=()=>{
  ...
  const included=[
    'Full Policy Tracker — live quarterly updates across all 12 budget pillars',
    'All 12 sector intelligence briefs, complete editions',
    '174+ venture models with capital ranges & IRR targets',
    'Budget execution monitor with milestone alerts',
    'Legislative tracker — pending bills & compliance guidance',
    'Government contact intelligence — ministry leads & directors',
    'Partnership window alerts with engagement scripts',
    'Co-financing eligibility assessments — DBG, GIIF, WDB',
    'Sankofa Initiative engagement playbook',
    'Cross-sector integration maps',
    'Quarterly analyst briefings — live calls',
    'Direct analyst Q&A access',
  ];
  const faqs=[
    {q:'Is this only for diaspora investors?',a:'No. BRIDGE membership serves diaspora investors, Ghana-based entrepreneurs, and development finance professionals. If you need decision-grade Ghana market intelligence, you belong here.'},
    {q:'What does applying involve?',a:'A short intake form covering background, sector interests, and investment capacity. Reviewed within 5 business days. Most qualified applicants are admitted.'},
    {q:'How is this different from this public tracker?',a:'This Public Edition covers the policy landscape. The Members Edition adds venture models, government contacts, co-financing assessments, engagement scripts, and quarterly live analyst briefings — the operational layer on top of the intelligence.'},
  ];
  return(
    <div id="sec-members" style={{background:C.ink,position:'relative',overflow:'hidden'}}>
      <div style={{position:'absolute',right:'-20px',bottom:'-20px',fontFamily:F.display,fontSize:'clamp(100px,18vw,260px)',fontWeight:900,color:'rgba(255,255,255,0.02)',pointerEvents:'none',userSelect:'none',letterSpacing:'-8px',lineHeight:1}}>MEMBERS</div>
      <div className="pad-section" style={{padding:'56px 64px 64px',position:'relative'}}>
        <div style={{maxWidth:'900px',margin:'0 auto'}}>

          {/* Header */}
          <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:'40px',alignItems:'end',marginBottom:'40px',paddingBottom:'32px',borderBottom:`1px solid rgba(255,255,255,0.06)`}}>
            <div>
              <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'16px'}}>
                <div style={{width:'28px',height:'1px',background:C.lime,opacity:0.7}}/>
                <span style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'3px',textTransform:'uppercase',color:C.lime,opacity:0.8}}>BRIDGE Members</span>
              </div>
              <h2 style={{fontFamily:F.display,fontSize:'clamp(24px,3.5vw,44px)',fontWeight:700,color:C.paper,lineHeight:1.08,margin:0}}>
                You have the public picture.<br/>
                <span style={{fontStyle:'italic',color:'rgba(250,248,243,0.32)',fontWeight:400}}>Members get the full intelligence.</span>
              </h2>
            </div>
          ...
          {/* Middle: package list + tiers */}
          <div className="two-col" style={{display:'grid',gridTemplateColumns:'1.05fr 0.95fr',gap:'40px',marginBottom:'32px'}}>

            {/* Package list */}
            <div>
              <div style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2.5px',textTransform:'uppercase',color:'rgba(250,248,243,0.22)',marginBottom:'16px'}}>Members Package — what's included</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 12px'}} className="upsell-tc">
                {included.map((item,i)=>(
                  <div key={i} style={{display:'flex',gap:'8px',alignItems:'flex-start',padding:'5px 0',borderBottom:`1px solid rgba(255,255,255,0.04)`}}>
            ...
            {/* Tiers */}
            <div className="upsell-col-r" style={{display:'flex',flexDirection:'column',gap:'0',borderLeft:`1px solid rgba(255,255,255,0.06)`,paddingLeft:'40px'}}>
              {/* Community */}
              <div style={{padding:'16px 0',borderBottom:`1px solid rgba(255,255,255,0.06)`,display:'flex',justifyContent:'space-between',alignItems:'center',gap:'12px'}}>
                <div>
                  <div style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:'rgba(250,248,243,0.22)',marginBottom:'3px'}}>Community</div>
                  <div style={{fontFamily:F.sans,fontSize:'11px',color:'rgba(250,248,243,0.3)',fontWeight:300}}>Policy overview · Calendar · Sector matrix</div>
                </div>
                <span style={{fontFamily:F.sans,fontSize:'10px',fontWeight:600,color:'rgba(250,248,243,0.22)',flexShrink:0}}>You are here</span>
              </div>
              {/* Network Member */}
              <div style={{padding:'22px 16px',margin:'16px 0',borderLeft:`2px solid ${C.lime}`,background:'rgba(184,217,53,0.05)'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'8px'}}>
                  <div>
                    <div style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:C.lime,marginBottom:'4px'}}>Network Member</div>
                    <div style={{fontFamily:F.sans,fontSize:'10px',color:'rgba(250,248,243,0.3)',fontWeight:300}}>Selective admission · 5-day review</div>
                  </div>
                  <div style={{textAlign:'right',flexShrink:0}}>
                    <div style={{fontFamily:F.display,fontSize:'20px',fontWeight:700,color:C.paper}}>$100</div>
                    <div style={{fontFamily:F.sans,fontSize:'9px',color:'rgba(250,248,243,0.3)'}}>– $500 / year</div>
                  </div>
                </div>
                <p style={{fontFamily:F.body,fontSize:'12px',color:'rgba(250,248,243,0.45)',lineHeight:1.7,fontWeight:300,margin:'0 0 14px'}}>
                  Full platform access, all 12 sector intelligence briefs, venture pipeline, analyst briefings, and co-financing assessments — across all 12 sectors.
                </p>
                <a href="#" style={{display:'inline-flex',alignItems:'center',gap:'8px',background:C.lime,color:C.ink,padding:'10px 22px',fontFamily:F.sans,fontSize:'11px',fontWeight:800,letterSpacing:'0.3px',textDecoration:'none'}}>
                  Apply for membership →
                </a>
              </div>
              {/* Contributor / Investor */}
              <div style={{padding:'16px 0',borderTop:`1px solid rgba(255,255,255,0.06)`,display:'flex',justifyContent:'space-between',alignItems:'center',gap:'12px'}}>
                <div>
                  <div style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:'rgba(250,248,243,0.22)',marginBottom:'3px'}}>Contributor / Investor</div>
                  <div style={{fontFamily:F.sans,fontSize:'11px',color:'rgba(250,248,243,0.3)',fontWeight:300}}>Deal flow · LP rights · Advisory Council · $500–$1M+</div>
                </div>
                <a href="#" style={{fontFamily:F.sans,fontSize:'10px',fontWeight:600,color:'rgba(250,248,243,0.22)',textDecoration:'none',flexShrink:0}}>Custom →</a>
              </div>
            </div>
          </div>

          {/* Bottom: FAQ + CTA */}
          <div style={{display:'grid',gridTemplateColumns:'1.05fr 0.95fr',gap:'40px',paddingTop:'28px',borderTop:`1px solid rgba(255,255,255,0.06)`}} className="two-col">
            {/* FAQ */}
            <div>
              <div style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2.5px',textTransform:'uppercase',color:'rgba(250,248,243,0.22)',marginBottom:'14px'}}>Questions</div>
              {faqs.map((f,i)=>(
                <div key={i} style={{borderBottom:`1px solid rgba(255,255,255,0.05)`}}>
                  <button onClick={()=>setObjOpen(objOpen===i?null:i)} style={{width:'100%',padding:'11px 0',background:'transparent',border:'none',cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'16px'}}>
                  ...
                  {objOpen===i&&(<p style={{fontFamily:F.body,fontSize:'12px',lineHeight:1.8,color:'rgba(250,248,243,0.35)',fontWeight:300,paddingBottom:'12px',margin:0}}>{f.a}</p>)}
                </div>
              ))}
            </div>
            {/* CTA */}
            <div className="upsell-col-r" style={{paddingLeft:'40px',display:'flex',flexDirection:'column',justifyContent:'center',gap:'12px'}}>
              <p style={{fontFamily:F.body,fontSize:'13px',fontStyle:'italic',color:'rgba(250,248,243,0.3)',lineHeight:1.7,margin:0,fontWeight:300}}>
                The complete BRIDGE intelligence portfolio — sector briefs, venture models, and quarterly analyst briefings — updated as Ghana's 2026 budget execution unfolds.
              </p>
              <a href="#" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',border:`1px solid rgba(255,255,255,0.1)`,color:'rgba(250,248,243,0.3)',padding:'11px 0',fontFamily:F.sans,fontSize:'10px',fontWeight:600,letterSpacing:'0.5px',textDecoration:'none'}}>
                Download this summary
              </a>
              <div style={{display:'flex',justifyContent:'center',gap:'16px'}}>
                {['Selective admission','5-day review','No spam'].map((t,i)=>(<span key={i} style={{fontFamily:F.sans,fontSize:'9px',color:'rgba(250,248,243,0.15)',letterSpacing:'0.3px'}}>{t}</span>))}
              </div>
            </div>
          </div>
...
const MobAccordion = ({label,hint,children,dark=false}) => {
  ...
  return (
    <div style={{background:dark?C.ink:C.paper,borderBottom:`1px solid ${dark?'rgba(255,255,255,0.07)':C.border}`}}>
      <button onClick={()=>setOpen(o=>!o)} style={{width:'100%',padding:'14px 16px',background:'transparent',border:'none',cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'12px'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'8px',flexShrink:0}}>
          <span style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'1px',textTransform:'uppercase',color:open?(dark?C.lime:C.forest):(dark?'rgba(250,248,243,0.3)':C.muted)}}>{open?'Close':'Open'}</span>
...
const MobileShell = () => {
    ...
    <div style={{padding:'0 0 80px'}}>
      <div style={{background:C.ink,padding:'24px 16px 20px',marginBottom:'1px'}}>
        <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'12px'}}>
          ...
          <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'3px',textTransform:'uppercase',color:C.lime}}>Public Edition · Policy Tracker</span>
        </div>
        <h1 style={{fontFamily:F.display,fontSize:'28px',fontWeight:900,color:C.paper,lineHeight:1.05,marginBottom:'10px'}}>
          Ghana Policy Tracker<br/><span style={{color:C.lime}}>2025–2026</span>
        </h1>
        <p style={{fontFamily:F.body,fontSize:'13px',lineHeight:1.7,color:'rgba(250,248,243,0.55)',fontWeight:300,marginBottom:'20px'}}>
          Living document covering the 2026 Budget, Sankofa Initiative, Digital Economy Policy, and sector-specific regulations.
        </p>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1px',background:'rgba(255,255,255,0.06)'}}>
          {[{v:String(POLICIES.length),l:'Policies'},{v:'All 12',l:'Sectors'},{v:'Mar 2026',l:'Updated'},{v:'Jun 2026',l:'Next Review'}].map((s,i)=>(
            <div key={i} style={{background:C.ink,padding:'14px 16px'}}>
              <div style={{fontFamily:F.mono,fontSize:'20px',fontWeight:500,color:C.lime,marginBottom:'2px'}}>{s.v}</div>
      ...
      {/* What this covers */}
      <MobAccordion label="What This Tracker Covers" hint={`${POLICIES.length} policies · 7 categories`}>
        {POLICIES.map((p,i)=>(
          <button key={p.id} onClick={()=>{setSheet({type:'policy',data:p});}} style={{display:'flex',justifyContent:'space-between',alignItems:'center',width:'100%',textAlign:'left',background:'transparent',border:'none',borderBottom:i<POLICIES.length-1?`1px solid ${C.border}`:'none',padding:'9px 0',cursor:'pointer',gap:'8px'}}>
            <div>
              <div style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:C.faint,marginBottom:'2px'}}>{p.category}</div>
              <div style={{fontFamily:F.sans,fontSize:'12px',fontWeight:700,color:C.ink,lineHeight:1.3}}>{p.title}</div>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:'6px',flexShrink:0}}>
              <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'1px',textTransform:'uppercase',color:(STATUS[p.status]||STATUS['Monitoring']).text}}>{p.status}</span>
      ...
      {/* Upgrade prompt */}
      <div style={{margin:'16px',background:C.forest,padding:'16px'}}>
        <div style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2.5px',textTransform:'uppercase',color:C.lime,marginBottom:'6px'}}>Members Edition</div>
        <p style={{fontFamily:F.body,fontSize:'13px',color:'rgba(250,248,243,0.7)',lineHeight:1.6,fontWeight:300,marginBottom:'12px'}}>
          Venture models, co-financing assessments, analyst briefings, and government contact intelligence — for $100–$500/year.
        </p>
        <button onClick={()=>setActiveTab(4)} style={{width:'100%',background:C.lime,color:C.ink,border:'none',padding:'11px',fontFamily:F.sans,fontSize:'11px',fontWeight:800,cursor:'pointer',letterSpacing:'0.3px'}}>
          See membership →
        </button>
      </div>
    </div>
  ...
  const TabPolicies = () => (
    <div style={{padding:'0 0 80px'}}>
      ...
      <div style={{background:C.paperDark,borderBottom:`1px solid ${C.border}`,padding:'10px 16px'}}>
        <div style={{display:'flex',gap:'6px',overflowX:'auto',scrollbarWidth:'none',paddingBottom:'2px'}}>
          {['ALL',...CATEGORIES].map(cat=>(
            <button key={cat} onClick={()=>setCatFilter(cat)} style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'1px',textTransform:'uppercase',padding:'5px 12px',whiteSpace:'nowrap',flexShrink:0,background:catFilter===cat?C.forest:'transparent',color:catFilter===cat?C.lime:C.muted,border:`1px solid ${catFilter===cat?C.forest:C.border}`,cursor:'pointer'}}>{cat==='ALL'?'All':cat}</button>
          ))}
        </div>
      </div>
      ...
      {filtered.map((p,i)=>{
        const s=STATUS[p.status]||STATUS['Monitoring'];
        return(
          <button key={p.id} onClick={()=>setSheet({type:'policy',data:p})} style={{display:'block',width:'100%',textAlign:'left',background:C.paper,border:'none',borderBottom:`1px solid ${C.border}`,padding:'14px 16px',cursor:'pointer'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'5px',gap:'8px'}}>
              <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:C.faint}}>{p.category}</span>
              <div style={{display:'flex',alignItems:'center',gap:'5px',flexShrink:0}}>
                <span style={{width:'5px',height:'5px',borderRadius:'50%',background:s.dot,display:'inline-block',flexShrink:0}}/>
                <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'1px',textTransform:'uppercase',color:s.text}}>{p.status}</span>
              </div>
            </div>
            <div style={{fontFamily:F.sans,fontSize:'14px',fontWeight:700,color:C.ink,lineHeight:1.3,marginBottom:'4px'}}>{p.title}</div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{fontFamily:F.body,fontSize:'11px',fontStyle:'italic',color:C.muted}}>{p.subtitle}</span>
              <span style={{fontFamily:F.mono,fontSize:'12px',color:C.faint,flexShrink:0,marginLeft:'8px'}}>›</span>
            </div>
          </button>
        );
      })}
  ...
  const TabCalendar = () => {
    const events=[
      {date:'Nov 2025',event:'Digital Credit Services Directive: licensing open',policy:'DCD',status:'Enacted'},
      {date:'Jan 2026',event:'2026 National Budget: implementation begins',policy:'Budget 2026',status:'Active'},
      {date:'Q1 2026',event:'1D1F Working Capital Fund: disbursement window',policy:'1D1F',status:'Pending'},
      {date:'Q1 2026',event:'MoFA Oil Palm Window: application period',policy:'Agri Modernisation',status:'Pending'},
      {date:'Q2 2026',event:'NHIA Digital Claims RFP: procurement launch',policy:'NHIS Reform',status:'Pending'},
      {date:'Q2 2026',event:'Sankofa Diaspora Bond: prospectus expected',policy:'Sankofa',status:'Pending'},
      {date:'Jun 2026',event:'Digital Credit Directive: compliance deadline',policy:'DCD',status:'Announced'},
      {date:'Q3 2026',event:'AfCFTA Phase 2 tariff schedule review',policy:'AfCFTA',status:'Monitoring'},
      {date:'Q3 2026',event:'Warehouse Receipt System: legal framework gazette',policy:'Agri Modernisation',status:'Pending'},
      {date:'Q4 2026',event:'BRIDGE Policy Tracker: next full review',policy:'Internal',status:'Monitoring'},
    ];
    return(
      <div style={{padding:'16px 16px 80px'}}>
        <div style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2.5px',textTransform:'uppercase',color:C.muted,marginBottom:'16px'}}>Key Dates · 2025–2026</div>
        {events.map((e,i)=>{
          const s=STATUS[e.status]||STATUS['Monitoring'];
          return(
            <div key={i} style={{display:'flex',gap:'12px',alignItems:'flex-start',padding:'11px 0',borderBottom:i<events.length-1?`1px solid ${C.border}`:'none'}}>
              <div style={{fontFamily:F.mono,fontSize:'11px',fontWeight:500,color:C.forest,flexShrink:0,minWidth:'60px',paddingTop:'1px'}}>{e.date}</div>
              <div style={{flex:1,minWidth:0}}>
                <p style={{fontFamily:F.sans,fontSize:'12px',color:C.ink,lineHeight:1.4,marginBottom:'3px',margin:'0 0 3px'}}>{e.event}</p>
                <span style={{fontFamily:F.mono,fontSize:'9px',color:C.faint}}>{e.policy}</span>
              </div>
  ...
  const TabSectors = () => (
    <div style={{padding:'16px 16px 80px'}}>
      <div style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2.5px',textTransform:'uppercase',color:C.muted,marginBottom:'16px'}}>Sector Policy Coverage</div>
      {POLICIES.map((p,pi)=>(
        <div key={pi} style={{padding:'12px 0',borderBottom:`1px solid ${C.border}`}}>
          <div style={{fontFamily:F.sans,fontSize:'12px',fontWeight:700,color:C.ink,marginBottom:'6px',lineHeight:1.3}}>{p.title}</div>
          <div style={{display:'flex',gap:'4px',flexWrap:'wrap'}}>
            {p.sectors.map(si=>(
              <span key={si} style={{fontFamily:F.mono,fontSize:'8px',fontWeight:700,color:C.forest,background:'rgba(27,77,62,0.08)',padding:'3px 7px',borderRadius:'2px'}}>
                {String(si+1).padStart(2,'0')} {SECTORS_SHORT[si]}
      ...
      <div style={{marginTop:'12px',fontFamily:F.body,fontSize:'11px',fontStyle:'italic',color:C.faint}}>Each tag indicates direct BRIDGE sector strategy or venture pipeline relevance.</div>
    </div>
  );
  ...
  const TabMembers = () => {
    const [objOpen,setObjOpen] = useState(null);
    const faqs=[
      {q:'Is this only for diaspora investors?',a:'No. BRIDGE serves diaspora investors, Ghana-based entrepreneurs, and development finance professionals.'},
      {q:'What does applying involve?',a:'A short intake form. Reviewed within 5 business days. Most qualified applicants are admitted.'},
      {q:'How is this different from this public tracker?',a:'This Public Edition covers the policy landscape. Members Edition adds venture models, government contacts, co-financing assessments, and quarterly analyst briefings.'},
    ];
    return(
      <div style={{padding:'0 0 80px'}}>
        <div style={{background:C.ink,padding:'24px 16px'}}>
          <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'14px'}}>
            ...
            <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'3px',textTransform:'uppercase',color:C.lime}}>BRIDGE Members</span>
          </div>
          <h2 style={{fontFamily:F.display,fontSize:'26px',fontWeight:700,color:C.paper,lineHeight:1.1,marginBottom:'10px'}}>
            You have the public picture.<br/>
            <span style={{fontStyle:'italic',color:'rgba(250,248,243,0.35)',fontWeight:400}}>Members get the full intelligence.</span>
          </h2>
          <div style={{borderTop:'1px solid rgba(255,255,255,0.07)',marginTop:'20px',paddingTop:'16px'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
              <div>
                <div style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:'rgba(250,248,243,0.25)',marginBottom:'2px'}}>Community</div>
                <div style={{fontFamily:F.sans,fontSize:'11px',color:'rgba(250,248,243,0.3)',fontWeight:300}}>This tracker · Newsletter · Public events</div>
              </div>
              <span style={{fontFamily:F.sans,fontSize:'9px',fontWeight:600,color:'rgba(250,248,243,0.2)'}}>You are here</span>
            </div>
            <div style={{borderLeft:`2px solid ${C.lime}`,padding:'14px',margin:'14px 0',background:'rgba(184,217,53,0.05)'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'8px'}}>
                <div>
                  <div style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:C.lime,marginBottom:'2px'}}>Network Member</div>
                  <div style={{fontFamily:F.sans,fontSize:'11px',color:'rgba(250,248,243,0.3)',fontWeight:300}}>Selective · 5-day review</div>
                </div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontFamily:F.display,fontSize:'20px',fontWeight:700,color:C.paper}}>$100</div>
                  <div style={{fontFamily:F.sans,fontSize:'8px',color:'rgba(250,248,243,0.3)'}}>– $500/yr</div>
                </div>
              </div>
              <p style={{fontFamily:F.body,fontSize:'12px',color:'rgba(250,248,243,0.45)',lineHeight:1.65,fontWeight:300,marginBottom:'12px'}}>
                Full platform, all 12 sector briefs, venture pipeline, co-financing assessments, quarterly analyst briefings.
              </p>
              <a href="#" style={{display:'block',textAlign:'center',background:C.lime,color:C.ink,padding:'11px',fontFamily:F.sans,fontSize:'11px',fontWeight:800,letterSpacing:'0.3px',textDecoration:'none'}}>Apply for membership →</a>
            </div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderTop:'1px solid rgba(255,255,255,0.07)'}}>
              <div>
                <div style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:'rgba(250,248,243,0.25)',marginBottom:'2px'}}>Contributor / Investor</div>
                <div style={{fontFamily:F.sans,fontSize:'11px',color:'rgba(250,248,243,0.3)',fontWeight:300}}>Deal flow · LP rights · $500–$1M+</div>
              </div>
              <a href="#" style={{fontFamily:F.sans,fontSize:'9px',fontWeight:600,color:'rgba(250,248,243,0.2)',textDecoration:'none'}}>Custom →</a>
            </div>
          </div>
        </div>
        {/* FAQ */}
        <div style={{padding:'0 16px'}}>
          <div style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2.5px',textTransform:'uppercase',color:C.muted,padding:'16px 0 8px'}}>Questions</div>
          {faqs.map((f,i)=>(
            <div key={i} style={{borderBottom:`1px solid ${C.border}`}}>
              <button onClick={()=>setObjOpen(objOpen===i?null:i)} style={{width:'100%',padding:'11px 0',background:'transparent',border:'none',cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'12px'}}>
                <span style={{fontFamily:F.sans,fontSize:'12px',fontWeight:600,color:C.ink,textAlign:'left',lineHeight:1.4}}>{f.q}</span>
                <span style={{fontFamily:F.sans,fontSize:'16px',fontWeight:300,color:C.faint,flexShrink:0,transform:objOpen===i?'rotate(45deg)':'none',transition:'transform 0.2s',display:'block'}}>+</span>
              </button>
              {objOpen===i&&<p style={{fontFamily:F.body,fontSize:'12px',lineHeight:1.75,color:C.muted,fontWeight:300,paddingBottom:'12px',margin:0}}>{f.a}</p>}
            </div>
          ))}
        </div>
      </div>
  ...
  const BottomSheet = () => {
    ...
    const {type,data:p} = sheet;
    const s = type==='policy' ? STATUS[p.status]||STATUS['Monitoring'] : null;
    return(
      <>
        <div onClick={closeSheet} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:400}}/>
        <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:500,background:C.paper,borderRadius:'12px 12px 0 0',maxHeight:'85vh',overflow:'hidden',display:'flex',flexDirection:'column',boxShadow:'0 -4px 32px rgba(0,0,0,0.2)'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 16px 12px',borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
            <span style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:C.muted}}>Policy Detail</span>
          ...
          <div style={{overflowY:'auto',flex:1,padding:'16px',WebkitOverflowScrolling:'touch'}}>
            <div style={{display:'flex',gap:'7px',alignItems:'center',flexWrap:'wrap',marginBottom:'8px'}}>
              <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:C.faint}}>{p.category}</span>
              {s&&<span style={{display:'inline-flex',alignItems:'center',gap:'5px',background:s.bg,padding:'3px 9px',borderRadius:'2px'}}><span style={{width:'5px',height:'5px',borderRadius:'50%',background:s.dot,display:'inline-block'}}/><span style={{fontFamily:F.sans,fontSize:'9px',fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:s.text}}>{p.status}</span></span>}
            </div>
            <h3 style={{fontFamily:F.display,fontSize:'20px',fontWeight:700,color:C.ink,lineHeight:1.2,marginBottom:'4px'}}>{p.title}</h3>
            <div style={{fontFamily:F.body,fontSize:'11px',fontStyle:'italic',color:C.muted,marginBottom:'14px'}}>{p.subtitle}</div>
            <div style={{background:C.paperDark,padding:'8px 12px',display:'flex',gap:'14px',flexWrap:'wrap',borderBottom:`1px solid ${C.border}`,marginBottom:'14px'}}>
              {[{l:'Enacted',v:p.enacted},{l:'Effective',v:p.effective},{l:'Updated',v:p.lastUpdated}].map((m,i)=>(
                <div key={i} style={{display:'flex',gap:'5px',alignItems:'center'}}>
                  <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'1px',textTransform:'uppercase',color:C.faint}}>{m.l}</span>
                  <span style={{fontFamily:F.mono,fontSize:'10px',color:C.ink}}>{m.v}</span>
                </div>
            ...
            <div style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:C.muted,marginBottom:'8px'}}>Overview</div>
            <p style={{fontFamily:F.body,fontSize:'13px',lineHeight:1.8,color:C.ink,fontWeight:300,marginBottom:'14px'}}>{p.summary}</p>
            <div style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:C.muted,marginBottom:'8px'}}>Key Figures</div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'14px'}}>
              {p.keyFigures.map((f,i)=>(
                <div key={i} style={{padding:'8px',background:C.paperDark}}>
                  <div style={{fontFamily:F.sans,fontSize:'8px',color:C.faint,marginBottom:'3px'}}>{f.label}</div>
                  <div style={{fontFamily:F.mono,fontSize:'12px',fontWeight:500,color:C.forest}}>{f.value}</div>
                </div>
              ))}
            </div>
            <div style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:C.muted,marginBottom:'8px'}}>First Provision</div>
            {p.provisions.slice(0,2).map((pr,i)=>(
              <div key={i} style={{border:`1px solid ${C.border}`,padding:'10px 12px',background:C.paperDark,marginBottom:'6px'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'4px',gap:'8px'}}>
                  <span style={{fontFamily:F.sans,fontSize:'11px',fontWeight:700,color:C.ink,flex:1}}>{pr.title}</span>
                  <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:pr.impact==='HIGH'?C.positive:pr.impact==='MEDIUM'?C.amber:C.muted,background:pr.impact==='HIGH'?'rgba(26,107,47,0.1)':'rgba(184,143,10,0.1)',padding:'2px 6px'}}>{pr.impact}</span>
                </div>
                <p style={{fontFamily:F.body,fontSize:'11px',color:C.muted,lineHeight:1.6,fontStyle:'italic',margin:0}}>{pr.detail}</p>
              </div>
            ))}
            <div style={{padding:'10px 12px',background:'rgba(27,77,62,0.04)',display:'flex',gap:'10px',alignItems:'flex-start',marginTop:'8px'}}>
              <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:C.lime,background:C.forest,padding:'3px 8px',flexShrink:0}}>Watch</span>
              <p style={{fontFamily:F.body,fontSize:'11px',fontStyle:'italic',color:C.muted,lineHeight:1.7,margin:0}}>{p.watch}</p>
            </div>
  ...
  const tabs=[TabOverview,TabPolicies,TabCalendar,TabSectors,TabMembers];
  ...
  return(
    <div style={{position:'fixed',inset:0,background:C.paper,display:'flex',flexDirection:'column',fontFamily:F.body,overflow:'hidden'}}>
      ...
      <div style={{background:C.ink,padding:'10px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0,borderBottom:'2px solid rgba(184,217,53,0.2)'}}>
        ...
        <span style={{fontFamily:F.sans,fontSize:'8px',fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:C.muted,background:'rgba(255,255,255,0.05)',border:`1px solid rgba(255,255,255,0.08)`,padding:'3px 8px'}}>Public Edition</span>
      </div>
      {/* Tab strip */}
      <div style={{background:C.paperDark,borderBottom:`1px solid ${C.border}`,display:'flex',flexShrink:0,overflowX:'auto',scrollbarWidth:'none'}}>
        {TABS.map((tab,i)=>(
          <button key={tab.id} onClick={()=>setActiveTab(i)} style={{flex:'0 0 auto',padding:'10px 16px',background:'transparent',border:'none',borderBottom:activeTab===i?`2px solid ${C.lime}`:'2px solid transparent',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:'3px',transition:'all 0.15s'}}>
...
export default function PolicyTracker(){
  ...
  return(
    <div className="nav-clearance" style={{fontFamily:F.body,background:C.paper,paddingBottom:'68px'}}>
      ...
      <div className="no-print pad-section" style={{background:C.paperDark,borderBottom:`1px solid ${C.border}`,padding:'10px 64px'}}>
